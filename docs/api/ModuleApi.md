...
```javascript
await xky.mousedown(0.5,0.5);//按下屏幕中间位置
await xky.mouseup(0.5,0.5);//释放屏幕中间位置
```
返回结果：
```javascript
{msg: "鼠标释放", errcode: 0}
...
* **用法：** `xky.copy()`
* **说明：** 复制，将选定内容复制到手机上剪贴板，类似Windows下的ctrl+c。
* **参数：** 无
...
* **用法：** `xky.cut()`
* **说明：** 剪切，将选定内容剪切到手机上剪贴板，类似Windows下的ctrl+x。
* **参数：** 无
...
* **用法：** `xky.paste()`
* **说明：** 粘贴，将手机上剪贴板中的内容粘贴出来，类似Windows下的ctrl+v。
* **参数：** 无
...
例子：
```javascript
// 不使用正则表达式查找文字为 微信 的控件
// 若是超过1000毫秒还没找到，就直接跳过执行下一步。
...
* **说明：** 从(startx,starty)的位置滑动多少次到达(endx,endy)的位置，x y均为百分比取值范围是0-1，可配合xiakeuispy工具获取坐标点。
* **参数：**
...
参数 | 值类型 | 说明
------------ | ------------- | -------------
model| string| Optional, the value is the model you want to specify, for example: mi5, nexus6, etc. For details, please refer to the support list.
...
参数 | 值类型 | 说明
------------ | ------------- | -------------
base64| string| 文件的base64代码
savepath| string| 文件在网盘里的路径

例子：
```javascript
await xky.uploadBase64("base64代码","aaa.jpb");
```
返回结果：
```javascript
{
